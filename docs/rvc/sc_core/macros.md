 ### macros.sv file
 This file has some macros that we use in our design. **That file is common to some other Mafia RTL cores and that the reason its relatively long**. We will explain only the relevant macros for the `sc_core` cpu.   

 This file is located in `/common/macros.sv` folder    
...
- Because we use `sc_core,` than the only sequential element we use is for `PC` register and Register file, the macro that used for Pc is:
